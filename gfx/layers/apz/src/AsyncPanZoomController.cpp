...
nsEventStatus AsyncPanZoomController::OnPanBegin(
  ...
  MOZ_ASSERT(block);
  // A block synthesized from a zero-delta pan-end that arrived with no active
  // block represents a gesture that has already ended, so there is nothing for
  // it to interrupt. Cancelling here would destroy a running scroll-snap or
  // overscroll snap-back animation and leave the scroll position stranded.
  if (block->WasSynthesizedFromZeroDeltaPanEnd()) {
    APZC_LOG_DETAIL(
        "OnPanBegin: block synthesized from a zero-delta pan-end, not "
...
nsEventStatus AsyncPanZoomController::OnPan(
  ...
  if (InScrollAnimation(ScrollAnimationKind::SmoothMsd)) {
    ...
    PanGestureBlockState* block = GetCurrentPanGestureBlock();
    if (block && block->WasSynthesizedFromZeroDeltaPanEnd()) {
      // A block synthesized from a zero-delta pan-end that arrived with no
