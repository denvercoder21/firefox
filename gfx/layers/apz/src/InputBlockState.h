...
class PanGestureBlockState : public CancelableBlockState {
 public:
  ...
  void SetWasSynthesizedFromZeroDeltaPanEnd() {
    mSynthesizedFromZeroDeltaPanEnd = true;
  }
  bool WasSynthesizedFromZeroDeltaPanEnd() const {
    return mSynthesizedFromZeroDeltaPanEnd;
  }
 ...
 private:
  ...
  // True if this block was synthesized from a zero-delta pan-end that arrived
  // with no active block, i.e. it represents a gesture that has already
  // finished.
  bool mSynthesizedFromZeroDeltaPanEnd;
  ScrollDirections mAllowedScrollDirections;
};
